...
def main(args):
    ...
    with open(args.output, 'a') as f:
        f.write('-' * 80 + '\n')
        f.write('Average across all tIoUs \n')
        for metric in evaluator.scores:
            score = evaluator.scores[metric]
            f.write('| %s: %2.4f \n' % (metric, 100 * sum(score) / float(len(score))))
...
if __name__=='__main__':
    ...
    parser.add_argument('-o', '--output', type=str, default='results.txt', help='result output file')
